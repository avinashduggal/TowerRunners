...
class ObstacleTowerEnv(gym.Env):
    ...
    def __init__(
        ...
        # Instance variables that was added to the class to test reward shaping methods
        # Plan - output the values stored in the info variable to see what could be used for checking the current floor and keys collected
        self.keys_collected = 0         # Starts with 0 keys
        self.current_floor = 1          # Starts on Floor 1
        ...
        # Output behavior_spec and action_shape, the number of branches of action
        # print(f"Behavior spec: {behavior_spec}")
        # print(f"Action shape: {behavior_spec.action_shape}")
    ...
    def reset(self, config=None):
        ...
        Returns: observation (object/list): the initial observation of the
            space.
        """
        # Reset these instance variables for the next episode
    ...
    def step(self, action):
        ...
        # Check what values are stored in the info variable returned by single_step
        # print(f"Brain info: {info['brain_info']}")
        # print(f"Total keys: {info['total_keys']}")
        # print(f"Current floor: {info['current_floor']}")

        # Implement positive rewards
        # Reward for picking up keys
        if info["total_keys"] > self.keys_collected:
            reward += 2.5
            self.keys_collected = info["total_keys"]
            print(f"Keys collected: {self.keys_collected}")
        
        # Reward for ascending to higher floor
